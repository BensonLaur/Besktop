...
`BESKTOP_COMBAT_DIRECTOR_PREVIEW=1` 用于全量漫游中的受控互动预览。Director 会等所有演员觉醒后先保留约 `6` 秒纯漫游，再从自然接近、空间充足且近期参与较少的演员中选择一对，在工作区内预约安全区域并稳定轮换四个固定场景。非参与演员若旧目标或行进路线穿过 reservation，会使用带滞回的目标重选和切向绕行平滑避让；一轮结束后双方短暂停留、向外分开，再按个人与全局冷却恢复普通漫游。同一时间最多一对演员互动；当前没有生命值、伤害数值、淘汰、连招、多人围攻或阵营关系。
