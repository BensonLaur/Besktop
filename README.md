...
当前已用固定双演员诊断预览接通靠近、对齐、共享时间线、真实 Contact 几何与结果反馈；普通 Release 已接入个体驱动的首版相遇基础。纯逻辑层可以同时接受多组互不冲突的相遇请求，但场景运行时仍处于单实例迁移阶段，每次只消费仲裁结果中的第一组，不会开启全量群架。
...
独立 `EncounterArbiter` 只检查演员占用、工作区、任务栏安全区域和 reservation 冲突，不负责随机点名，也没有固定活动对数上限；一次仲裁可以接受零个、一个或多个互不冲突请求。当前 `IconFightScene` 仍只把第一个接受结果交给单实例 `EncounterDirector`，继续复用注意、减速接近、面对彼此、观察、攻防/退让/虚张声势和结果余波。命中、重击、拨挡、闪避和打空仍使用既有 `CombatPair`；非战斗分支不启动攻击动作、Contact 或命中特效。下一阶段会用 `ActiveEncounterPool` 保存多个独立相遇，再下一阶段才扩展约 `8–16` 秒、`3–7` 次交换的完整交锋段。按 `P` 关闭后不再产生新请求，已开始的相遇仍自然完成并释放 reservation。
