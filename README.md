...
6. 第一批生态就绪后再保留约 6 秒纯探索，随后局部演员可能并行发生相遇；每场仍只有两名演员，可能进行多次攻防交换、退让或虚张声势，其他演员继续闲逛或完成觉醒。
...
当前已用固定双演员诊断预览接通靠近、对齐、共享时间线、真实 Contact 几何与结果反馈；普通 Release 已接入个体驱动的首版相遇基础。`CombatPair` 表示一次完整攻防交换，产品相遇中的 `CombatEpisode` 则把通常 `3–7` 次、以 `4–6` 次为主的交换组织成一场完整交锋；连同相遇前奏和最终余波，目标时长约 `8–16` 秒。`ActiveEncounterPool` 会消费仲裁结果中的全部安全请求，使多个互不冲突的局部相遇和交锋段独立运行；没有固定活动对数上限，实际并发仅由双方握手、演员占用和空间条件决定，也不会开启全量群架。

非参与演员也会从个体视角感知附近相遇：事件只提供不含桌面语义的中心、阶段、Contact 结果和强度快照，每个演员再结合距离、视野、稳定倾向与临时状态独立决定忽略、短暂注意、在 reservation 外观察或主动避让。没有固定全局旁观人数；不同并行事件可以拥有不同的局部观察者，观察者不会加入 `CombatEpisode`，事件结束后会经过短恢复与个体冷却再回到普通漫游。
...
独立 `EncounterArbiter` 只检查演员占用、工作区、任务栏安全区域和 reservation 冲突，不负责随机点名，也没有固定活动对数上限；一次仲裁可以接受零个、一个或多个互不冲突请求。`ActiveEncounterPool` 为每个接受结果保存稳定 ID、双方索引、reservation、独立 `EncounterDirector` 与 `CombatEpisode`，逐场更新和释放；非参与演员会避开全部活动 reservation。每个 Episode 独立维护稳定随机、主动方、场景、体力、压力和主动权，在完整恢复后重整 `0.3–0.8` 秒，再根据拨挡、闪避、打空、轻击或重击结果决定反击、继续或结束；同一演员最多连续主动两次，重击只低频出现在后半段。最终只触发一次余波和离场。按 `P` 关闭后立即停止新请求，正在交锋的 Episode 完成当前交换后自然收尾，不强行凑足原计划次数；重新开启仍先经过漫游缓冲。固定 `BESKTOP_COMBAT_PREVIEW` 继续直接运行单次 `CombatPair`，便于逐动作检查。当前仍不包含血量、伤害数字、死亡、淘汰或三人以上围攻。
