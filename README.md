...
6. 第一批生态就绪后再保留约 6 秒纯探索，随后画面中偶尔只有一对演员相遇；它们可能攻防，也可能退让或虚张声势，其他演员继续闲逛或完成觉醒。
...
产品 Director 不再等待所有演员觉醒；当第一批全部完成四肢生长且至少有两名正常 Wanderer 后，才开始既有约 `6` 秒纯探索缓冲，再从自然接近、空间充足且近期参与较少的演员中选择一对，在工作区内预约安全区域。独立 `EncounterDirector` 让双方依次经历注意、减速接近、面对彼此和约 `0.6–1.0` 秒观察，再按稳定权重选择约 `60%` 攻防、`25%` 退让或 `15%` 虚张声势；只有攻防分支进入既有 `CombatPair`。命中、重击、拨挡、闪避和打空会生成不同停顿与离场顺序，非战斗分支不会启动攻击动作、Contact 或命中特效。尚未觉醒或仍在长四肢的演员不会成为候选，后续觉醒波次也不会因互动而停止。非参与演员若旧目标或行进路线穿过 reservation，会使用带滞回的目标重选和切向绕行平滑避让；一轮结束后双方按个人与全局冷却恢复普通漫游。同一时间最多一对演员互动；当前没有生命值、伤害数值、淘汰、连招、多人围攻或阵营关系。按 `P` 只切换自动互动，已开始的三种相遇会自然完成并释放 reservation；再次开启会先保留约 `4` 秒漫游缓冲。
