...
Besktop 的首个玩法叫 **Icon Fight**：它会把你的桌面临时变成一个安全的全屏舞台，让空间分散的首批桌面图标先醒来探索，随后通过第二批、邻近加速和兜底波次让全部图标陆续长出白色手脚并自由闲逛。第一轮单演员基础动作库已经完成；普通 Release 会在纯漫游开场后，偶尔选择一对自然靠近的演员完成一次攻防，再让它们回到闲逛。
...
6. 第一批生态就绪后再保留约 6 秒纯探索，随后画面中偶尔只有一对演员进行一次短暂攻防，其他演员继续闲逛或完成觉醒。
...
当前已用固定双演员诊断预览接通靠近、对齐、共享时间线、真实 Contact 几何与结果反馈；轻量 `CombatDirector` 已成为普通 Release 的首版默认体验，但同时最多只控制一对演员，不会开启全量群架。
...
固定双演员预览支持 `lead_parry`、`lead_slip`、`uppercut_light_hit`、`side_kick_heavy_hit`。推荐设置 `BESKTOP_MAX_ACTORS=2`；双方觉醒后会平滑走到工作区中央附近、连续转身面对彼此、稳定站立，再按共享时间线完成交互并平滑回到站位循环。Contact 使用实际拳端或脚端和防守者身体轴胶囊判定；防守窗口不会绕过几何条件。固定 Combat、Turn 和 Action 预览会按该顺序优先接管场景，并暂停产品 Director。未设置固定预览时，普通 Release 默认运行全量觉醒、自由漫游和低频单对互动；当前仍没有生命值、胜负或全量群架。

觉醒计划由独立 `AwakeningDirector` 在舞台初始化时一次性生成：约 `20%` 的演员作为空间分散的第一批，在约 `1.2–3.4` 秒启动；约 `35%` 作为第二批，在约 `3.6–7.8` 秒启动；其余演员在约 `7.2–14.8` 秒兜底醒来。已完成四肢生长并进入探索的演员，会在约 `2.7` 个图标边长范围内，以稳定的 `0.45–0.85` 秒短延迟提前附近沉睡演员的计划时间。该规则不依赖名称、路径或墙钟随机数，也不会改变漫游随机序列。

产品 Director 不再等待所有演员觉醒；当第一批全部完成四肢生长且至少有两名正常 Wanderer 后，才开始既有约 `6` 秒纯探索缓冲，再从自然接近、空间充足且近期参与较少的演员中选择一对，在工作区内预约安全区域并稳定轮换四个固定场景。尚未觉醒或仍在长四肢的演员不会成为候选，后续觉醒波次也不会因互动而停止。非参与演员若旧目标或行进路线穿过 reservation，会使用带滞回的目标重选和切向绕行平滑避让；一轮结束后双方短暂停留、向外分开，再按个人与全局冷却恢复普通漫游。同一时间最多一对演员互动；当前没有生命值、伤害数值、淘汰、连招、多人围攻或阵营关系。按 `P` 只切换自动互动，分波觉醒始终继续；再次开启会先保留约 `4` 秒漫游缓冲。
