...
Besktop 的首个玩法叫 **Icon Fight**：它会把你的桌面临时变成一个安全的全屏舞台，让全部桌面图标像小角色一样错峰醒来、长出白色手脚并自由闲逛。第一轮单演员基础动作库已经完成；普通 Release 会在纯漫游开场后，偶尔选择一对自然靠近的演员完成一次攻防，再让它们回到闲逛。
...
6. 开场漫游约 6 秒后，画面中偶尔只有一对演员进行一次短暂攻防，其他演员继续闲逛。
...
产品 Director 会等所有演员觉醒后先保留约 `6` 秒纯漫游，再从自然接近、空间充足且近期参与较少的演员中选择一对，在工作区内预约安全区域并稳定轮换四个固定场景。非参与演员若旧目标或行进路线穿过 reservation，会使用带滞回的目标重选和切向绕行平滑避让；一轮结束后双方短暂停留、向外分开，再按个人与全局冷却恢复普通漫游。同一时间最多一对演员互动；当前没有生命值、伤害数值、淘汰、连招、多人围攻或阵营关系。按 `P` 关闭时，正在进行的一轮会自然完成后再回到纯漫游；再次开启会先保留约 `4` 秒漫游缓冲。
