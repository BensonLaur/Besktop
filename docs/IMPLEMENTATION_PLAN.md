...
- 受控互动导演已接入诊断路径：`CombatDirector` 等全部演员觉醒后先保留 `6` 秒纯漫游，再按距离、参与历史、最近配对和安全空间选择一对，创建工作区内 reservation 并复用 `CombatPair` 完成一轮交互。非参与演员的旧路径会用带 `0.85` 秒滞回的切向目标重选绕开 reservation；结果结束后停留约 `0.45` 秒、分开、释放并进入 `14` 秒个人冷却与 `7–11` 秒全局冷却，演员不超过 `6` 个时额外降频。同一时间最多一对，关闭诊断开关时默认全量纯漫游保持不变；这些集中参数仍可随人工视觉验收继续微调。
...
- `BESKTOP_COMBAT_DIRECTOR_PREVIEW=1` 在全量漫游中启用单对受控互动，优先级高于转身和单动作预览；固定 `BESKTOP_COMBAT_PREVIEW` 存在时仍以固定场景预览为准。
