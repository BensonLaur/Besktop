...
- 受控互动导演已接入首版产品路径：`CombatDirector` 等全部演员觉醒后先保留 `6` 秒纯漫游，再按距离、参与历史、最近配对和安全空间选择一对，创建工作区内 reservation 并复用 `CombatPair` 完成一轮交互。非参与演员的旧路径会用带 `0.85` 秒滞回的切向目标重选绕开 reservation；结果结束后停留约 `0.45` 秒、分开、释放并进入 `14` 秒个人冷却与 `7–11` 秒全局冷却，演员不超过 `6` 个时额外降频。同一时间最多一对。按 `P` 可仅在当前窗口关闭或恢复自动互动；活动轮次不会被硬切，再次启用先保留 `4` 秒纯漫游。固定诊断预览仍受总开关保护并优先接管场景；这些集中参数仍可随人工视觉验收继续微调。
