...
- 分波觉醒调度已从场景拆到纯逻辑 `AwakeningDirector`：初始化时按稳定种子和原始位置一次性生成约 `20% / 35% / 其余` 的第一批、第二批与兜底批次，时间范围分别约为 `1.2–3.4 / 3.6–7.8 / 7.2–14.8` 秒。第一批使用确定性最远点采样覆盖不同桌面区域；已完成四肢生长的 Wanderer 可在约 `2.7` 个图标边长内，以稳定 `0.45–0.85` 秒延迟提前附近沉睡演员，但所有演员即使未被路过也会由兜底批次唤醒。
- 受控互动导演已接入首版产品路径：`CombatDirector` 在第一批全部完成四肢生长且至少两名演员正常漫游后，才开始 `6` 秒纯探索缓冲，不再等待全员；随后按距离、参与历史、最近配对和安全空间选择一对，创建工作区内 reservation 并复用 `CombatPair` 完成一轮交互。尚未觉醒、正在觉醒或长四肢的演员不会成为候选，后续波次也不会因互动停止。非参与演员的旧路径会用带 `0.85` 秒滞回的切向目标重选绕开 reservation；结果结束后停留约 `0.45` 秒、分开、释放并进入 `14` 秒个人冷却与 `7–11` 秒全局冷却，演员不超过 `6` 个时额外降频。同一时间最多一对。按 `P` 只关闭或恢复自动互动，不控制觉醒；活动轮次不会被硬切，再次启用先保留 `4` 秒纯漫游。固定诊断预览仍受总开关保护并优先接管场景。
...
- `BESKTOP_COMBAT_DIRECTOR_PREVIEW=1` 显式启用 Director 诊断观测；普通 Release 的低频单对互动本身默认开启。固定 Combat、Turn 和 Action 预览会暂停产品 Director。
- `CombatDirector` 负责受控配对和空间预约；当前不直接开启全量群架，也没有生命值或胜负。演员不足、空间不足或没有候选时保持纯漫游，不弹出错误。
