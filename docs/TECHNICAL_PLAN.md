...
阶段 A 已落地独立的 `action_clip.*` 与 `action_player.*`：动作标识、统一五阶段、内置只读 clip、姿态采样、每演员播放状态和一次性事件均不依赖窗口、桌面采集或 Explorer。场景每演员每帧只推进/采样一次动作，再把局部身体旋转、根偏移和手部 IK 目标叠加到现有基础姿态；未启用预览时采样通道保持为零。
