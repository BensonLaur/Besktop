...
固定双演员诊断使用独立 `combat_pair.*` 维护共享时间线和结果状态，不把导演逻辑堆入渲染器。Contact 由 `IconFightScene` 在精确事件时刻复用同一套姿态、投影和骨架求解，生成拳端/脚端与目标身体轴胶囊，再交给纯数学结果判定；渲染缓存不是唯一判定来源。当前只处理固定前两名演员，不包含全量配对、生命值和胜负。
