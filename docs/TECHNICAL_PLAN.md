...
- 图标矩形位于 `z = 0` 平面，但图标几何中心相对身体中轴保留“半宽 + 小间隙”的水平偏移，不与转身枢轴重合，也不覆盖身体轴。
- 正面和背面都绘制同一个真实图标贴图。
...
向右稳定姿态作为统一局部骨架基准，向左由相对基准的 `180°` Y 轴旋转得到。身体中轴的屏幕位置在转身期间固定；图标中心作为带水平偏移的子节点绕该轴走连续圆弧，在 `90°` 附近经过中轴，完成后自然落到另一侧，而不是以图标中心为轴原地翻卡片。图标四角、肩胯挂点和四肢关节都经过同一身体中轴旋转及透视投影，不再在 `facing` 改变时直接翻转挂点 X 或前后深度。图标正反面继续绘制同一纹理；越过 `90°` 后只调整目标三角形顶点顺序以保持非镜像 UV，不翻转文件图标、文字或 Logo。肢体前后层依据投影后整条肢体的平均深度决定，深度关系在转身中段自然交换；相同输入产生稳定排序，不依赖离散的移动方向。演员初始化时保持图标真实捕获中心不变，并根据左右可用空间选择安全初始朝向，使靠边演员的后续圆弧优先向屏幕内侧展开。
