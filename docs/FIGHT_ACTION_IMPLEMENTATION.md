...
角色根节点、肩部中心和胯部中心定义身体竖直中轴，图标中心相对该轴保留“薄片半宽 + 小间隙”的水平偏移，避免薄片覆盖身体轴。图标平面、肩胯和四肢关节共同绕身体轴旋转；右向基准 yaw 为 `0°`，左向为 `180°`。图标中心从轴一侧沿连续圆弧移动到另一侧，`90°` 时经过身体轴附近，同时原始 48 像素平面投影宽度接近 `0`，绘制层保留 `2` 像素最小边缘。背面继续使用非镜像 UV 顶点顺序。前后肢体按投影后根、关节和末端的平均深度分层，而不是等到朝向提交时离散交换。初始图标中心仍保持真实捕获位置；若演员靠近左右边缘，则选择让完整转身圆弧朝屏幕内侧展开的初始朝向，避免转身时把图标甩出可见区。
