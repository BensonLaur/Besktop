...
其余第一轮动作继续使用同一组通用通道：前后手/脚独立 IK 目标、身体与下半身旋转、前向/侧向/纵向 root offset、拳击/踢击/受击/打空强度。渲染层只消费这些通道，不读取 `Hook`、`RoundhouseKick` 等动作 ID。48 像素纯数学测试中，`hook` Contact 肘角约 `65.4°`；`swing_punch` 的深度弧约为 `0.68 planeSide`，大于 hook 的 `0.51`；`front_kick` 膝角从约 `103.8°` 收膝伸展到约 `171.1°`。新增三种踢击的大腿/小腿继续固定为 `19.2 / 19.68` 像素，支撑脚局部目标漂移为 `0`；计入回旋踢有限下半身旋转后的近似投影漂移最大约 `1.26` 像素。`spinning_back_kick` 的动作内部 yaw 连续经过约 `90°` 并完成 `360°`，结束采样回到 `0°` 等价中性姿态，不写入或改变演员的外部 `TurnMotionState` 与 `currentFacing`。
